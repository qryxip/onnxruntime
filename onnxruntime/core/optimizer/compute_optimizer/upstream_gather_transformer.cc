...
bool UpStreamGatherGraphTransformer::UpStreamInternal(Graph& graph, std::deque<SliceInfo>& queue,
  ...
  if (!pass_through_config.actor->PreCheck(graph, current_node, *info_ptr, pass_through_config.input_indices, logger,
  ...
  for (auto pair : candidate_input_indices) {
    ...
    SliceInfo gather_info = PropagateSlicingForInput(graph, slice_node, current_node, input_index, *info_ptr, new_axis,
                                                     logger);
  ...
  ORT_ENFORCE(RemoveOriginSlicingOp(graph, slice_node, current_node, logger, *info_ptr).IsOK());
  if (!pass_through_config.actor->PostProcess(graph, current_node, index_of_output, info_ptr->non_negative_axis,
...
SliceInfo UpStreamGatherGraphTransformer::PropagateSlicingForInput(Graph& graph,
                                                                   Node& slice_node,
                                                                   Node& current_node,
...
Status UpStreamGatherGraphTransformer::RemoveOriginSlicingOp(Graph& graph, Node& slice_node, Node& current_node,
