...
OrtTensorTypeAndShapeInfo::Ptr GetTensorShapeAndTypeHelper(ONNXTensorElementDataType type, onnxruntime::TensorShape shape,
...
OrtTensorTypeAndShapeInfo::Ptr GetTensorShapeAndType(onnxruntime::TensorShape shape,
...
OrtTensorTypeAndShapeInfo::Ptr GetTensorShapeAndType(onnxruntime::TensorShape shape, const std::vector<std::string>* dim_params,
...
ORT_API_STATUS_IMPL(OrtApis::GetTensorTypeAndShape, _In_ const OrtValue* v, _Outptr_ OrtTensorTypeAndShapeInfo** out) {
  ...
  if (v->IsTensor() || v->IsSparseTensor()) {
    ...
    if (v->IsTensor()) {
      ...
      auto ptr = GetTensorShapeAndType(*shape, *data_type);
      *out = ptr.release();
    } else {
#if !defined(DISABLE_SPARSE_TENSORS)
...
ORT_API_STATUS_IMPL(OrtApis::GetSparseTensorValuesTypeAndShape, _In_ const OrtValue* v,
  ...
  auto ptr = GetTensorShapeAndType(values.Shape(), *values.DataType());
  *out = ptr.release();
  return nullptr;
#else
...
ORT_API_STATUS_IMPL(OrtApis::GetSparseTensorIndicesTypeShape, _In_ const OrtValue* v,
  ...
  auto ptr = GetTensorShapeAndType(indices_tensor.Shape(), *indices_tensor.DataType());
  *out = ptr.release();
  return nullptr;
#else
  ORT_UNUSED_PARAMETER(v);
